...
之所以讨论超参数，本质上是通过不同的超参数组合，找到最优的模型系数。需要说明的是，也只有这些模型参数会被用于预测，超参数并不在模型中展示。用用我们的实验作为实例，在预测股价走势上，我们对预测结果是0或1我们并没有倾向性，所以此处将超参数调优的目标scoring设定为准确率。我们使用GridSearchCV的函数获得不同组合下的模型结果，具体调优的超参数包括：penalty = ['none', "l1", "l2"]、C = [0.001, 0.01, 0.05, 0.1, 0.5, 1, 5, 10, 25] 。
